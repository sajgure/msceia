...
class Report_web extends Base_Controller 
{
	...
	*/
    ...
    public function certificate($id)
    {
        ...
        $data['inst_data_jan23']=$this->common_model->selectDetailsWhr('view_institute_jan23','institute_id',$id);
        $this->load->view('certificate',$data);
    }
    public function certificate_pdf($id,$limit)
    {
        $data['stud_result']=$this->report_model->passStudData($id,$limit);
       
        $pdfname=$data['stud_result'][0]->institute_name;
        $html=$this->load->view('certificate_pdf',$data,TRUE);
        $this->report_creation->create_pdf($html,$pdfname);
    }
    public function ssd_certificate_pdf($id,$limit)
    {
        $data['stud_result']=$this->report_model->ssd_passStudData($id,$limit);
        $pdfname=$data['stud_result'][0]->institute_name;
        $html=$this->load->view('ssd_certificate_pdf',$data,TRUE);
        $this->report_creation->create_pdf($html,$pdfname);
    }
    public function pass_student_list($id)
    {
        $id = $this->encryptdecryptstring->decrypt_string($id);
        $data['ssd_stud_data'] =$this->report_model->ssd_pass_student_details($id);
        $data['stud_data'] =$this->report_model->pass_student_details($id);
