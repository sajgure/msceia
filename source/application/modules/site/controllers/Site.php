...
class Site extends Base_Controller 
{
    ...
    public function privecy_policy()
    {
        ...
        $data['appname'] = $this->config->load('app-config', TRUE)['AppName'];
        $data['keydata'] = $this->common_model->selectDetailsWhr('tbl_rest_keys','id',1);
        $this->load->view('privecy-policy',$data);
    }
    ...
    public function certificates()
    {
        ...
        $data['favicon'] = $this->config->load('app-config', TRUE)['favicon'];
        $data['appname'] = $this->config->load('app-config', TRUE)['AppName'];
        $data['inst_data_oct23']=$this->common_model->selectDetailsWhr('view_institute_2025_s1','institute_id',$institute_id);
        // echo $this->db->last_query();die;
        // print_r($data['inst_data_oct23']);die;
        $this->load->view('certificates',$data);
    }

    public function certificates_pdf($id,$limit)
    {
        $data['stud_result']=$this->report_model->passStudData($id,$limit,9);
        $pdfname=$data['stud_result'][0]->institute_name;
        $html=$this->load->view('certificates_pdf',$data,TRUE);
        $this->report_creation->create_pdf($html,$pdfname);
    }
    public function ssd_certificates_pdf($id,$limit)
    {
        $data['stud_result']=$this->report_model->ssd_passStudData($id,$limit,8);
        $pdfname=$data['stud_result'][0]->institute_name;
        $html=$this->load->view('ssd_certificates_pdf',$data,TRUE);
        $this->report_creation->create_pdf($html,$pdfname);
    }
    public function certificates_pdf_batch($batch,$id,$seat)
    {
        $data['stud_result']=$this->report_model->passStud_singleData($batch,$id,$seat);
        // echo $this->db->last_query();die;
        $pdfname=$data['stud_result'][0]->seat_no;
        $html=$this->load->view('certificates_pdf',$data,TRUE);
        $this->report_creation->create_pdf($html,$pdfname);
    }
}
